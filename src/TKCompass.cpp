...
/* Private variables ---------------------------------------------------------*/
volatile uint8_t TKCompass::_keyPin = 0;
volatile uint8_t TKCompass::_ledPin = 0;
 ...
 */
TKCompass::TKCompass(uint8_t port) : TKPort(port)
{
  Device_Address = COMPASS_DEFAULT_ADDRESS;
  Calibration_Flag = false;
}
 ...
 */
TKCompass::TKCompass(uint8_t port, uint8_t address) : TKPort(port)
{
 ...
 */
TKCompass::TKCompass(uint8_t keyPin, uint8_t ledPin)
{
  Device_Address = COMPASS_DEFAULT_ADDRESS;
  Calibration_Flag = false;
  _keyPin = keyPin;
 ...
 *   address - the i2c address you want to set
 */
TKCompass::TKCompass(uint8_t keyPin, uint8_t ledPin, uint8_t address)
{
  Device_Address = address;
  Calibration_Flag = false;
  _keyPin = keyPin;
 ...
 */
void TKCompass::setpin(uint8_t keyPin, uint8_t ledPin)
{
  _keyPin = keyPin;
  _ledPin = ledPin;
#ifdef TK_PORT_DEFINED
...
void TKCompass::begin(void)
{
  ...
  // write MODE register
  TKasurement_Mode = COMPASS_MODE_SINGLE;
  writeReg(COMPASS_RA_MODE, TKasurement_Mode);
 ...
 *   testConnection
 * \par Description
 *   Identify the device whether is the TKCompass.
 * \param[in]
 *   None
 * \par Output
 *   None
 ...
 */
bool TKCompass::testConnection(void)
{
 ...
 * \par Others
 *   Will return a correct angle when you keep the TKCompass working in the plane which have calibrated.
 */
double TKCompass::getAngle(void)
{
  int16_t cx,cy,cz;
 ...
 */
int16_t TKCompass::getHeadingX(void)
{
  int8_t return_value = 0;
  deviceCalibration();
  ...
  if( TKasurement_Mode == COMPASS_MODE_SINGLE )
  {
 ...
 *   The sensor value is a 16 bits signed integer.
 */
int16_t TKCompass::getHeadingY(void)
{
  int8_t return_value = 0;
  deviceCalibration();
 ...
 *   The sensor value is a 16 bits signed integer.
 */
int16_t TKCompass::getHeadingZ(void)
{
  int8_t return_value = 0;
  deviceCalibration();
 ...
 */
int16_t TKCompass::getHeading(int16_t *x, int16_t *y, int16_t *z)
{
  int8_t return_value = 0;
  deviceCalibration();
 ...
 */
int8_t TKCompass::writeReg(int16_t reg, uint8_t data)
{
  int8_t return_value = 0;
 ...
 */
int8_t TKCompass::writeData(uint8_t start, const uint8_t *pData, uint8_t size)
{
  int8_t return_value = 0;
 ...
 */
int8_t TKCompass::readData(uint8_t start, uint8_t *buffer, uint8_t size)
{
  int16_t i = 0;
  int8_t return_value = 0;
  Wire.beginTransmission(Device_Address);
 ...
 *   deviceCalibration
 * \par Description
 *   Calibration function for the TKCompass. 
 * \param[in]
 *   None
 * \par Output
 *   None
 ...
 *   rotate the TKCompass over 360 degress in a stable plane that you specified to calibrate,
 *   and press the button again to finish the calibration.
 */
void TKCompass::deviceCalibration(void)
{
#ifdef TK_PORT_DEFINED
...
#endif
  {
    ...
    if(digitalRead(_keyPin) == 0)
      ...
      {
#ifdef COMPASS_SERIAL_DEBUG
        Serial.println("It is not TK Compass!!!!!");
#endif
        return;
      }  
 ...
 */
void TKCompass::read_EEPROM_Buffer(void)
{
 ...
 *   Call the arduino official EEPROM library.
 */
void TKCompass::write_EEPROM_Buffer(struct Compass_Calibration_Parameter *parameter_pointer)
{
