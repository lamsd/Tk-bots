 ...
 *    None
 */
TK4Button::TK4Button(void) : TKPort()
{
 ...
 */
TK4Button::TK4Button(uint8_t port) : TKPort(port)
{
  _KeyPin = s2;
 ...
 */
TK4Button::TK4Button(uint8_t port)
{
  _KeyPin = port;
  Pre_Button_Value = 950;
}
 ...
 *    None
 */
void TK4Button::setpin(uint8_t port)
{
  _KeyPin = port;
#ifdef TK_PORT_DEFINED
  s2 = port;
 ...
 */
uint8_t TK4Button::pressed(void)
{
  ...
  if (current_time - previous_time > DEBOUNCED_INTERVAL)
  {
    ...
    // smaller number in TK4Button.h.
    previous_time = current_time;
#ifdef TK_PORT_DEFINED
    key_temp_value = TK4Button::aRead2();
#else  // TK_PORT_DEFINED
