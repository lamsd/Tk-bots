 ...
 */
TKRGBLed::TKRGBLed(uint8_t port) : TKPort(port)
{
 ...
 */
TKRGBLed::TKRGBLed(uint8_t port, uint8_t led_num) : TKPort(port)
{
  pinMask       = digitalPinToBitMask(s2);
  ws2812_port   = portOutputRegister(digitalPinToPort(s2) );
  //set pinMode OUTPUT */
 ...
 *   led_num - The LED number
 */
TKRGBLed::TKRGBLed(uint8_t port, uint8_t slot, uint8_t led_num) : TKPort(port)
{
  if(slot == SLOT1)
  {
 ...
 *   port - arduino port
 */
TKRGBLed::TKRGBLed(uint8_t port)
{
 ...
 *   led_num - The LED number
 */
TKRGBLed::TKRGBLed(uint8_t port, uint8_t led_num)
{
  pinMask       = digitalPinToBitMask(port);
  ws2812_port   = portOutputRegister(digitalPinToPort(port) );
  // set pinMode OUTPUT */
 ...
 *   None
 */
void TKRGBLed::reset(uint8_t port)
{
  _port = port;
  _slot = SLOT2;
  s2    = mePort[port].s2;
 ...
 *   None
 */
void TKRGBLed::reset(uint8_t port,uint8_t slot)
{
  _port = port;
  _slot = slot;
  s2    = mePort[port].s2;
 ...
 *   None
 */
void TKRGBLed::setpin(uint8_t port)
{
  setColor(0,0,0,0);
  fillPixelsBak(0,2,1);
  pinMask   = digitalPinToBitMask(port);
 ...
 *   None
 */
void TKRGBLed::setNumber(uint8_t num_leds)
{
  count_led = num_leds;
 ...
 */
cRGB TKRGBLed::getColorAt(uint8_t index)
{
  cRGB px_value;
 ...
 *   The index value from 1 to the max
 */
uint8_t TKRGBLed::getNumber(void)
{
  return(count_led);
}
 ...
 *   None
 */
void TKRGBLed::fillPixelsBak(uint8_t red, uint8_t green, uint8_t blue)
{
 ...
 */
bool TKRGBLed::setColorAt(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index < count_led)
  {
    uint8_t tmp = index * 3;
 ...
 */
bool TKRGBLed::setColor(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index == 0)
  {
    for(int16_t i = 0; i < count_led; i++)
 ...
 */
bool TKRGBLed::setColor(uint8_t red, uint8_t green, uint8_t blue)
{
 ...
 *   The index value from 1 to the max, if you set the index 0, all the LED will be lit
 */
bool TKRGBLed::setColor(uint8_t index, long value)
{
  if(index == 0)
  {
    for(int16_t i = 0; i < count_led; i++)
 ...
 *   None
 */
void TKRGBLed::rgbled_sendarray_mask(uint8_t *data, uint16_t datlen, uint8_t maskhi, uint8_t *port)
{
